 ...
 * override** (`elevationOverridden`): that is the one state where every point is
 * known to be DEM-derived, so inserting DEM-sampled points between vertices adds
 * no recorded-vs-DEM seam. A track's own recorded elevation is left alone (even
 * when full) rather than have DEM injected between its measured points, as are
 * *fill missing* and *keep recorded*. It is also a no-op for a dense track (no
 * long segments to subdivide), leaving consumers reading `trackGeojson`.
 */
export async function ensureRenderGeojson(
  getState: () => RootState,
  dispatch: Dispatch<RootAction>,
): Promise<void> {
  const { trackGeojson, renderTrackGeojson, elevationOverridden } =
    getState().trackViewer;

  if (renderTrackGeojson || !trackGeojson || !elevationOverridden) {
    return;
  }
