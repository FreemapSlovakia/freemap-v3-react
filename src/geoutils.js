...
export function formatGpsCoord(angle, cardinals, style = 'DMS', language) {
  ...
  switch (style) {
    case 'DMS': {
      ...
      const seconds = Intl.NumberFormat(language || 'en', { minimumFractionDigits: 3, maximumFractionDigits: 3 })
        .format((a - degrees - minutes / 60) * 3600);
      return `${cardinal}${degrees}° ${minutes}' ${seconds}"`;
    }
    case 'DM': {
      const degrees = Math.floor(a);
      const minutes = Intl.NumberFormat(language || 'en', { minimumFractionDigits: 4, maximumFractionDigits: 4 })
        .format((a - degrees) * 60);
      return `${cardinal}${degrees}° ${minutes}'`;
    }
    case 'D': {
      return `${cardinal}${Intl.NumberFormat(language || 'en', { minimumFractionDigits: 6, maximumFractionDigits: 6 }).format(a)}°`;
    }
    default: {
