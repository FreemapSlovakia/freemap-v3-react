...
- **Keep comments brief. Default to one or two lines, hard cap three.** Say what isn't obvious from the code; don't restate the code, enumerate every related value, or explain the reasoning at essay length. A comment is a hint to whoever comes next — a new developer or a new agent session — not a record of how the decision was reached. Only exceed three lines where a future session would otherwise break the code: a non-obvious constraint, an ordering that matters, an API quirk being worked around — and then state the trap, not the story around it.
  ...
  - Cut design rationale and UX reasoning. If it explains *why this is nice* rather than *what will bite you*, it belongs in the commit message or in `doc/`, not in the source.
