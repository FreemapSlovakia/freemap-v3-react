...
- **Also add the key's type to `src/translations/messagesInterface.ts`** — this is a hand-maintained (NOT generated) `Messages` type that `en.tsx` and every generated `<lang>.tsx` are typed against. Omitting it makes `tsc` fail on `en.tsx` ("may only specify known properties") and on each plain locale ("missing properties").
- `<lang>.template.tsx` files use `DeepPartialWithRequiredObjects<Messages>`; missing keys fall back to English.
- The plain `<lang>.tsx` files are generated by `pnpm sync-language-files` (runs as part of `pnpm start` and `pnpm build`) and will be overwritten. They are gitignored, so regeneration is invisible in `git status` but still required for `tsc` to pass.
