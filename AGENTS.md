...
- **Keep comments brief.** A line or two saying what isn't obvious from the code; don't restate the code, enumerate every related value, or explain the reasoning at essay length. A comment is a hint to whoever comes next — a new developer or a new agent session — not a record of how the decision was reached. Go longer only where a future session would otherwise break the code: a non-obvious constraint, an ordering that matters, an API quirk being worked around.
