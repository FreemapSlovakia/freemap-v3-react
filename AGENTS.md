...
- [Weather radar layer](./doc/weather-radar.md) — the animated `R` overlay over the LibreWXR (RainViewer-v2-compatible) API: the single-origin `LIBREWXR_URL` rule that lets a caching proxy be swapped in without a code change, the wire contract, the transient/persisted slice split, the polling processor's two deliberate exceptions (initial run, unawaited refresh), and the per-frame tile-layer pool with its cross-fade and pan-drop rules. **Read this before touching `src/features/weatherRadar/` or the radar layer's tile source.**
