...
- [GPS recorder integration](./doc/gps-recorder.md) — how the PWA drives the standalone Android recorder APK over its `127.0.0.1` HTTP API: the Android+Chromium+flag availability gate, the loopback/Local-Network-Access connection rules, the wire contract (`protocol.ts` is the single place to change it), the start-via-`intent://` flow, how the three failure causes are told apart, and the `/track?since=` + SSE catch-up. **Read this before touching `src/features/gpsRecorder/` or anything that talks to the recorder.**
