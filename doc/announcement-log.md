...
- **Video on Facebook is a lottery, not an upgrade.** Median reach by format: photos
  ...
  **But "post or reel" is not a choice we make** — attaching a video to an ordinary Page
  ...
  the feature *is* movement — a radar loop, a before/after sweep — a still cannot show it
...
|--------|---------|-------|---------|-------|
| lead | 🌍 Masto | **England 🏴󠁧󠁢󠁥󠁮󠁧󠁿 terrain** — hillshading and contours regenerated from the Environment Agency's 1 m LIDAR Composite DTM 2022, and premium elevation now reads the same model instead of GEDTM30. Open Government Licence v3. | `1e4f91ba` (attribution + premium country list; the shading/contours are a renderer-side deploy, no commit in this repo) | **Mastodon lead, not Facebook** — same reasoning as Norway below: foreign terrain is Facebook's worst performer, while datasets-and-licences posts are what this audience boosts. 1 m is the finest national model in the set, so a before/after over the Lake District or the Peak District sells it on detail alone. Credit "© Environment Agency copyright and/or database right 2022. All rights reserved." verbatim, `#England #OpenData #LIDAR`. No prior OSM-community intro post for the UK — this can double as one. |
...
|------|-------|-------|---------|------|
| 2026-08-18 | **Animated precipitation radar** — the `R` overlay: EUMETNET OPERA composite over Europe (Italy from the national Radar-DPC), own toolbar with play/pause, frame stepping and a timeline; 2 h of history free, 6 h with premium | 0♥ 1↻ (just posted) | `7ca9bef2` `d8be404c` `2a1a7369` `e0d11317` `6040b186` `dd847c90` `98871283` | https://en.osm.town/@FreemapSlovakia/117115042225932514 |
