...
| lead | both | **Offline maps became usable for real** — edit a cached map's area and zoom range instead of re-downloading (only the difference is fetched/deleted); download-size estimate samples real tiles at the real DPI plus a storage warning; while online, panning outside the cached area shows live tiles instead of error tiles; a cached map renders at the scale it was downloaded at. Offline maps live under **Manage maps** with a single Activate action, and cached tiles are served in Chromium browsers too. | `17f27fbe` `3d7a10e9` `ef0278b7` `1bb1cd19` `dd3e3c28` `7147f796` `508f3892` `736fd65b` `dde43a89` | **🇸🇰 FB done 2026-08-15**, as one post carrying the saved-route row and the offline-UX tail with it. **Mastodon half still open**, and weak there — practical rather than terrain/open-data, which is what that audience boosts; decide whether it is worth a post at all rather than assuming it is owed one. |
| lead | both | **A saved map carries its route** — the computed route and its elevation line are stored with the map, so it opens offline with the real route, profile, ascent/descent and elevation colorize, and no routing request. **Recompute route** in the ⋮ menu asks the router again. | `bff71a23` | Went out on Facebook 2026-08-15 inside the offline post; Mastodon half open. |
...
| tail | both | **Offline, the app says what it can do** — an offline mark beside the logo, and everything needing the server dims and explains itself instead of failing when clicked; the search box still answers coordinates, a bounding box, tile numbers and pasted GeoJSON. Saving a map now works offline too: it waits in an outbox and goes when the connection does. Also: a saved map carries its pinned results, unsaved edits survive a reload, and Load asks Append or Replace. | `f47c197c` `9ed9f391` `1898901b` `818eb550` `e81f338b` `20f76fbc` `61df754d` | A bullet under the offline-maps lead above, not a post — the app was usable offline before; this makes it honest about which parts are. The offline **save** is the one genuinely new capability here, so it is the bullet worth writing. Creating a map offline and deleting one still need a connection. The Facebook post of 2026-08-15 took the save and the pinned nálezy; the offline mark and the dimming have not been said anywhere. |
