...
| lead | both | **Offline maps became usable for real** — edit a cached map's area and zoom range instead of re-downloading (only the difference is fetched/deleted); download-size estimate samples real tiles at the real DPI plus a storage warning; while online, panning outside the cached area shows live tiles instead of error tiles; a cached map renders at the scale it was downloaded at. | `17f27fbe` `3d7a10e9` `ef0278b7` `1bb1cd19` `dd3e3c28` | Pairs naturally with the saved-route topic below into one "plan at home, ride without signal" post. **Seasonal: strongest in August** — holidays, mountains, no signal and roaming abroad. With radar on hold, this is the Facebook lead. |
| lead | both | **A saved map carries its route** — the computed route and its elevation line are stored with the map, so it opens offline with the real route, profile, ascent/descent and elevation colorize, and no routing request. **Recompute route** in the ⋮ menu asks the router again. | `bff71a23` | |
| lead | both | **Search holds more than one result** — any number of results on the map at once, hovering the list previews one, "Keep on the map" pins it, and the URL carries them all. The Objects tool hands one — or every visible object — over to search as lookups. | `77f12b0f` `8cc9a138` `4be61e6b` `b6ddc927` | |
| lead | 🌍 Masto | **Record a track from the web app** — the PWA drives the standalone Freemap GPS recorder (Android APK) over its loopback API, so a recording runs in the background with the screen off and the map shows it live. Since then: a live elevation profile while recording, elevation + satellite count in the readout, the toolbar stays collapsed during a recording, and Finish confirms before ending one. | `7616e552` `c8c9e171` `c5908bd8` `6af45d4b` `fda3f534` `803924c8` `27ecba06` | **Facebook already had it** (2026-07-30, 669 reach) — this is the Mastodon half, and it can carry the improvements that have landed since. Say **Android-only** in the post: the recorder is an APK and the feature is deliberately gated to Android, so an international audience will otherwise expect it on iOS. See [`gps-recorder.md`](./gps-recorder.md). |
...
| tail | both | **Elevation chart window is resizable** by a grip and remembers its geometry; the **legend** shows the zoom you are actually looking at. | `e562de6e` `8a421e42` | |
...
| 2026-07-27 | **Direction, distance and azimuth** — heading beam from GPS or compass, and a labelled line from your position to the crosshair | 3♥ 3↻ | `b4ea7537` | https://en.osm.town/@FreemapSlovakia/116991250866361203 |
...
| 2026-07-30 | Akože čože!? Nahrávanie trasy priamo vo webovej aplikácii www.freemap.sk? A čo by nie. S našou… | 669 reach · 12 react · 1 cmt · 2 link clicks | — | https://www.facebook.com/FreemapSlovakia/posts/pfbid028JngnzT41pxx6i7trnUZ6AnGC9xPWYbbCxKUjaiFS4qyGHeetH9e4kMQYWBLzoz7l |
| 2026-07-29 | Výškový profil je takmer raketová veda 🚀 Znie to ako triviálna vec: vezmi trasu, pozri sa do mo… | 1,580 reach · 33 react · 5 cmt · 1 shr | — | https://www.facebook.com/FreemapSlovakia/posts/pfbid06nmPis2sBQgfBq5rUSfj4N77RrydKYGp6NBfprkr3i7Uf3wu9MpdQAN6eYtSU2j3l |
