...
| lead | both | **Offline maps became usable for real** — edit a cached map's area and zoom range instead of re-downloading (only the difference is fetched/deleted); download-size estimate samples real tiles at the real DPI plus a storage warning; while online, panning outside the cached area shows live tiles instead of error tiles; a cached map renders at the scale it was downloaded at. Offline maps live under **Manage maps** with a single Activate action, and cached tiles are served in Chromium browsers too. | `17f27fbe` `3d7a10e9` `ef0278b7` `1bb1cd19` `dd3e3c28` `7147f796` `508f3892` `736fd65b` `dde43a89` | The offline lead: pair it with the saved-route row below and the offline-UX tail into one "plan at home, ride without signal" post. **Seasonal: strongest in August** — holidays, mountains, no signal and roaming abroad. With radar on hold, this is the Facebook lead. |
