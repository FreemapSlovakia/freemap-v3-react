...
|--------|---------|-------|---------|-------|
| lead | both | **The whole app works without a connection, and says so** — an offline mark beside the logo, and everything the server is needed for dims and explains itself up front instead of failing when clicked (the search box still answers coordinates, a bounding box, tile numbers and pasted GeoJSON). My Maps saves offline into an outbox and sends it on reconnect, keeping the map's offline copy in step; a saved map carries its pinned results, so it opens without fetching a single element; unsaved edits survive a reload; Load asks Append or Replace instead of a setting decided in advance. | `f47c197c` `9ed9f391` `1898901b` `818eb550` `e81f338b` `20f76fbc` `61df754d` | **These three offline rows are one post, not three** — with the two below (cached maps, a saved map's route) it is the whole "plan at home, ride without signal" story, and August is when it lands. The Facebook lead while radar is on hold. Creating a map offline and deleting one still need a connection — say so, or don't claim more than "save". |
