...
- **Hub (layer/tool) landing pages** — `seo.ts` holds curated copy distilled from `src/static/llms.txt` for the main map layers and tools (e.g. `layers=O`, `tool=route-planner`). Rendered in **sk + en** (`HUB_LANGS`) for now, with `WebPage` JSON-LD, canonical, Open Graph, and a cross-linked sibling list. Curated copy writes the domain name as the `{brand}` placeholder, which `renderHub` expands via `brand(lang)` to the page's home domain (`Freemap.sk` for sk/cs, `Freemap.eu` otherwise) — never hardcode a domain in the copy, or `freemap.eu` pages advertise the wrong site. _Expanding hubs to all 9 languages is pending — it needs the `Hub.title`/`description` records translated (≈19 hubs × title+description per language)._
