...
- **Hub (layer/tool/dialog) landing pages** — `hubs.ts` holds curated copy distilled from `src/static/llms.txt` for the main map layers, tools and dialogs (e.g. `layers=O`, `tool=route-planner`, `show=legend`). **26 hubs × all 9 UI languages = 234 pages** (`HUB_LANGS` is `LANGS`), with `WebPage` JSON-LD, canonical, Open Graph, and a cross-linked sibling list. Curated copy writes the two names as placeholders, expanded by `expandNames(text, lang)`: `{brand}` → the home domain (`Freemap.sk` for sk/cs, `Freemap.eu` otherwise) and `{site}` → the portal name (`Freemap Slovakia` / `Freemap Europe`). Never hardcode either in the copy, or `freemap.eu` pages advertise the wrong site. The same pair drives the `<title>` suffix, the nav breadcrumb, `og:site_name` and the JSON-LD `name` — in `objects.ts`'s POI pages too. The homepage `<title>`/description come from `src/translations/<lang>-shared.ts`, where each language's title already carries the name for its own domain.

  There is deliberately **no `layers=X` hub**: `fileName()` would render it as `layers=X&lang=<lang>`, which is exactly the homepage prerender's name — and that file is nginx's `try_files` fallback for every crawler request it cannot match. The homepage already is that page. `index.ts` enforces this rather than leaving it to a comment: homepages, hubs and documents all go through one `writePage`, which throws if two pages claim the same file name. That covers the homepage clash, two hubs sharing a `param`, and a hub shaped like a document URL — all of which would otherwise lose a page silently while leaving its URL in the sitemap. POI pages are deliberately outside it: a feature matching two categories legitimately writes the same page twice.

  There is also no hub for the GPS recorder: the tool is Android-only (`gpsRecorderAvailableSelector`), and `tool=` drops a tool the device cannot offer, so the page would have landed most searchers on a map with nothing opened.

  **Descriptions belong near 160 characters** — a search result truncates the tail, so a premium or coverage caveat put last is the part that disappears. Every hub whose copy carries such a caveat is held under that (the cadastre and `layers=Z` premium zooms, the radar band, the viewshed and panorama bare-earth limit, the credits cost of the offline export, and offline maps needing no other app); measure the copy **expanded**, since `{brand}` grows by three characters. Many hubs without a caveat still run to ~240 — mostly the non-`sk`/`en` translations, which came out longer than the English they were written from. Trimming those is a worthwhile follow-up, not a correctness problem.

  Note `premiumFromZoom` is *not* a minimum zoom — the layer works below it and is paywalled at and above it (`coords.z >= premiumFromZoom`). Reading it the other way is a mistake the cadastre copy shipped with once.

  Copy outside `sk`/`en` has not had a native review — the same caveat the POI page copy carries.
